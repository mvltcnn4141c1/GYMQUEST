...
import { calculateWorkoutXp } from '@/lib/workoutXp';
import {
  validateWorkout,
  type WorkoutMode,
...
export default function LogWorkoutScreen() {
  ...
  const params = useLocalSearchParams<{ exerciseType?: string; exerciseName?: string; isLiftoffTemplate?: string }>();
  const { userId, setCharacter, character, refreshCharacter } = useGame();
  ...
  }>({ visible: false, xp: 0, coins: 0, bonusXp: 0, title: "Tebrikler Usta!", fullScreenConfetti: false, soundType: "coin" });
  ...
  const isLiftoffTemplate = params.isLiftoffTemplate === 'true';
  ...
  const { data: remoteExercisesData } = useQuery<{ exercises: RemoteExercise[] }>({
    queryKey: ['exercises-by-category', selectedCategory],
    queryFn: () => apiGet(`/exercises/${selectedCategory}`),
  ...
  const boostMultiplier = Number(storeData?.activeBoosts?.[0]?.multiplier || 100);
  const ownedItemIds: string[] = storeData?.ownedItemIds || [];
  const equipmentMultiplier = ownedItemIds.reduce((acc: number, itemId: string) => {
    if (itemId === 'itm_dumbbell_basic' && selectedExercise?.category !== 'kardiyo') return acc + 0.05;
    if (itemId === 'itm_jump_rope_pro' && selectedExercise?.category === 'kardiyo') return acc + 0.05;
    if (itemId === 'itm_protein_shake') return acc + 0.1;
    return acc;
  }, 1);
  const classMasteryMultiplier = (() => {
    const cls = String(character?.class || '').toLowerCase();
    const cat = selectedExercise?.category;
    const strClasses = new Set(['warrior', 'fighter', 'barbarian']);
    const agiClasses = new Set(['rogue', 'archer', 'ranger', 'mage', 'wizard']);
    const endClasses = new Set(['paladin', 'cleric', 'druid', 'warlock']);
    const mapsTo = cat === 'kardiyo' ? 'AGI' : (cat === 'fullbody' ? 'END' : 'STR');
    if ((mapsTo === 'STR' && strClasses.has(cls)) || (mapsTo === 'AGI' && agiClasses.has(cls)) || (mapsTo === 'END' && endClasses.has(cls))) {
      return 1.1;
    }
    return 1;
  })();
  const xpEst = selectedExercise
    ? (selectedExercise.unit === 'reps'
      ? calculateWorkoutXp({
          ...
          boostMultiplier,
          reduced: suspicious || (healthResult ? !healthResult.hasMovement : false),
        })
      : getExerciseXP(
          selectedExercise.id,
          parseInt(sets) || 1,
          ...
          parseInt(duration) || 0,
          parseFloat(km) || 0,
          { boostMultiplier, classMultiplier: classMasteryMultiplier, equipmentMultiplier },
        ))
    : 0;
  const coinEst = Math.max(0, Math.floor(xpEst * 0.12));
  const calEst = selectedExercise
    ? getExerciseCal(
        selectedExercise.id,
        parseInt(sets) || 1,
        parseInt(reps) || 0,
  ...
  async function handleLog(disclaimerJustAccepted = false) {
    if (!selectedExercise || (!isDebugUser && cooldownRemaining > 0)) return;
  ...
  async function executeLog(health: HealthCheckResult, forceManual = false) {
    ...
    try {
      ...
      setVerificationStatus(result?.verificationStatus === "pending" ? "pending" : "verified");
      setVerificationMessage(
        result?.verificationMessage ||
        (result?.verificationStatus === "pending"
          ? "Analiz Ediliyor: Şüpheli Aktivite"
          : "Verified Workout"),
      );
      await AsyncStorage.setItem(LAST_WORKOUT_KEY, Date.now().toString());
      setCooldownRemaining(isDebugUser ? 0 : Math.ceil(COOLDOWN_MS / 1000));

      if (result.character) setCharacter(result.character);
      await queryClient.removeQueries({ queryKey: ['workouts', userId] });
      await queryClient.removeQueries({ queryKey: ['quests', userId] });
      await queryClient.removeQueries({ queryKey: ['character', userId] });
      ...
      if (result.leveledUp) {
        ...
        if (result?.isPendingApproval || result?.verificationStatus === "pending") {
          setRewardToast({
            visible: true,
            xp: 0,
            coins: 0,
            bonusXp: 0,
            title: 'Analiz Ediliyor: Supheli Aktivite',
            fullScreenConfetti: false,
            soundType: 'coin',
          });
        }
        const prevTier = String(character?.tier || '').toLowerCase();
        const nextTier = String(result?.character?.tier || character?.tier || '').toLowerCase();
        const tierChanged = !!prevTier && !!nextTier && prevTier !== nextTier;
        const title = tierChanged ? `KADEME ATLADIN: ${nextTier.toUpperCase()}!` : 'Tebrikler Usta!';
        setRewardToast({
          visible: true,
          xp: result.xpEarned || xpEst,
          coins: result.gymCoinsEarned || 0,
          bonusXp: Number(result.classMasteryBonusXp || 0),
          title,
          fullScreenConfetti: nextTier === 'mythic' && tierChanged,
          soundType: tierChanged ? 'levelup' : 'coin',
        });
        setXpOverlay({
          visible: true,
          xp: result?.verificationStatus === "pending" ? 0 : (result.xpEarned || xpEst),
          coins: result.gymCoinsEarned || 0,
          gems: 0,
        });
      }
  ...
  const isCoolingDown = !isDebugUser && cooldownRemaining > 0;
  ...
  const remoteExercises = remoteExercisesData?.exercises || [];
  const availableExercises = (remoteExercises.length > 0
    ? remoteExercises.map((ex) => ({
        ...(EXERCISE_MAP[ex.id] || {
          id: ex.id,
          name: ex.name,
          nameEn: ex.nameEn || ex.name,
          category: selectedCategory as ExerciseCategory,
          subMuscle: ex.subMuscle || 'Genel',
          icon: 'dumbbell',
          difficulty: 'orta' as const,
          equipment: ['yok'],
          joint: 'Coklu',
          instructions: [ex.howToTip || 'Formunu koruyarak kontrollu tekrar yap.'],
          unit: ex.unit || 'reps',
          maxReps: 100,
          maxSets: 20,
          maxMinutes: 180,
    ...
    : (EXERCISES_BY_CATEGORY[selectedCategory as ExerciseCategory] || []))
    .filter((ex) =>
  ...
  useEffect(() => {
    ...
    if (availableExercises.length === 0) {
      console.error('[LogWorkout] Bos egzersiz listesi', {
        selectedCategory,
        remoteCount: remoteExercises.length,
        fallbackCount: (EXERCISES_BY_CATEGORY[selectedCategory] || []).length,
      });
    }
  }, [selectedCategory, availableExercises.length, remoteExercises.length]);
  ...
  return (
    <View style={[styles.screen, { paddingTop: topPad }]}>
      <RewardToast
        ...
        bonusXp={rewardToast.bonusXp}
        title={rewardToast.title}
        fullScreenConfetti={rewardToast.fullScreenConfetti}
        soundType={rewardToast.soundType}
        onHide={() => setRewardToast((p) => ({ ...p, visible: false, fullScreenConfetti: false, bonusXp: 0 }))}
      />
        ...
        {selectedExercise && (
          <>
            ...
            <View style={styles.xpPreview}>
              ...
              <View style={styles.xpPreviewItem}>
                <MaterialCommunityIcons name="circle-multiple" size={22} color={COLORS.success} />
                <Text style={styles.xpPreviewLabel}>Tahmini Coin</Text>
                <Text style={[styles.xpPreviewValue, { color: COLORS.success }]}>+{coinEst}</Text>
              </View>
            </View>
            <Text style={styles.coinHint}>Kalori tahmini: ~{calEst} kal</Text>
      ...
      <View style={[styles.footer, { paddingBottom: botPad + 16 }]}>
        {!effortError && verificationStatus ? (
          <View
            style={[
              styles.verifyBanner,
              verificationStatus === "verified" ? styles.verifyBannerOk : styles.verifyBannerPending,
            ]}
          >
            <MaterialCommunityIcons
              name={verificationStatus === "verified" ? "check-circle" : "progress-clock"}
              size={16}
              color={verificationStatus === "verified" ? COLORS.success : COLORS.warning}
            />
            <Text
              style={[
                styles.verifyBannerText,
                { color: verificationStatus === "verified" ? COLORS.success : COLORS.warning },
              ]}
            >
              {verificationMessage}
...
const styles = StyleSheet.create({
  ...
  coinHint: { fontFamily: 'Inter_400Regular', fontSize: 11, color: COLORS.textMuted, marginTop: -6, marginBottom: 10 },
  ...
  verifyBannerOk: { backgroundColor: COLORS.success + '18', borderColor: COLORS.success + '40' },
  verifyBannerPending: { backgroundColor: COLORS.warning + '18', borderColor: COLORS.warning + '40' },
  verifyBannerText: { fontFamily: 'Inter_600SemiBold', fontSize: 13, flex: 1 },
  cooldownBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
