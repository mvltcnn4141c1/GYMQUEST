...
const SECTION_ITEMS_COIN = 'coin_items';
...
function ItemCard({ item, owned, equipped, isDebugUser, gymCoins, gems, characterLevel, onBuy, onEquip, activeBoosts }: ItemCardProps) {
  const locked = !isDebugUser && characterLevel < item.levelRequired;
  ...
  const canEquipAura = item.type === 'aura' && (owned || isDebugUser);
...
export default function StoreScreen() {
  ...
  const [localEquippedAura, setLocalEquippedAura] = useState<string | null>(null);
  const [debugEquippedAura, setDebugEquippedAura] = useState<string | null>(null);
  ...
  const isDebugUser = data?.isDebugUser === true || data?.isAdmin === true;

  React.useEffect(() => {
    if (isDebugUser) {
      if (debugEquippedAura === null) {
        setDebugEquippedAura(equippedAura);
      }
      return;
    }
    setLocalEquippedAura(equippedAura);
  }, [equippedAura, isDebugUser, debugEquippedAura]);
  const catalog = data?.catalog ?? [];
  const itemCatalog = data?.itemCatalog ?? [];
  const mergedCatalog = [...catalog, ...itemCatalog];

  const level = character?.level ?? 1;
  const onRefreshStore = useCallback(async () => {
    await queryClient.removeQueries({ queryKey: ['store', userId] });
  ...
  const sections = [
    ...
    { key: SECTION_ITEMS_COIN, label: 'Ekipman', icon: 'dumbbell' as const },
  ...
  const filteredItems = mergedCatalog.filter((item) => {
    if (activeSection === SECTION_ITEMS_COIN) return item.type === 'item' && item.currency === 'gym_coin';
    ...
    return false;
  }).map((item) => (isDebugUser ? { ...item, price: 1 } : item));
  ...
  async function handleBuy() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (msg.includes('yeterli gym coin yok') || msg.includes('yetersiz')) {
          Alert.alert('Coin Yetersiz', 'Daha fazla antrenman yapmalisin usta!');
        } else {
  ...
  async function handleEquip(itemId: string | null) {
    ...
    const previousAura = localEquippedAura;
    setLocalEquippedAura(itemId);
    if (isDebugUser) {
      setDebugEquippedAura(itemId);
      if (character) {
        setCharacter({ ...character, equippedAura: itemId });
      }
    }
    setIsEquipping(true);
    try {
      ...
      if (!res.ok) {
        setLocalEquippedAura(previousAura);
        if (isDebugUser) {
          setDebugEquippedAura(previousAura);
        }
  ...
  return (
    <View style={[styles.screen, { paddingTop: topPad }]}>
      ...
      {activeBoosts.length > 0 && (
        <ActiveBoostBanner boosts={activeBoosts} catalog={mergedCatalog} />
      )}
      ...
      {isLoading ? (
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={isRefetching} onRefresh={onRefreshStore} tintColor={COLORS.gold} />}
        >
        ...
        >
          {filteredItems.map((item) => (
            <ItemCard
              key={item.id}
              item={item}
              owned={ownedIds.includes(item.id) || (isDebugUser && item.type === 'aura')}
              equipped={(isDebugUser ? debugEquippedAura : localEquippedAura) === item.id}
