...
export default function HomeScreen() {
  ...
  const equippedAuraLabel = AURA_LABELS[String(character.equippedAura || '').toLowerCase()] || null;
  ...
  const safeCharacterName = character.name?.trim() || 'Isimsiz';
  const tierRaw = String(character.tier || 'common').toLowerCase();
  ...
  const tierLabel = tierLabelMap[tierRaw] || 'COMMON';
  const tierColor = tierColorMap[tierRaw] || tierColorMap.common;
  ...
  const safeClassLabel = (classLabels[character.class] || character.class || 'sinif').toUpperCase();
      ...
      >
        ...
        <View style={styles.characterCard}>
          {championBannerActive ? (
            <View style={styles.homeChampionBanner}>
              <MaterialCommunityIcons name="flag-checkered" size={13} color={COLORS.gold} />
              <Text style={styles.homeChampionBannerText}>SAMPIYON BANNER</Text>
            </View>
          ) : null}
          <View style={styles.cardRow}>
            <View style={styles.characterInfo}>
              <View style={styles.nameRow}>
                <Text style={styles.characterName}>{safeCharacterName}</Text>
                <View style={[styles.tierTag, { borderColor: `${tierColor}66`, backgroundColor: `${tierColor}1F` }]}>
              ...
              {equippedAuraLabel ? (
                <View style={styles.auraTag}>
                  <MaterialCommunityIcons name="star-four-points" size={10} color={COLORS.gold} />
                  <Text style={styles.auraTagText}>{equippedAuraLabel}</Text>
                </View>
              ) : null}
        ...
        <Pressable
          style={styles.liftoffBtn}
          onPress={() => router.push('/liftoff')}
        >
          <MaterialCommunityIcons name="rocket-launch" size={18} color={COLORS.background} />
          <Text style={styles.liftoffBtnText}>Quick Start (Liftoff)</Text>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  homeChampionBannerText: { fontFamily: 'Inter_700Bold', fontSize: 10, color: COLORS.gold, letterSpacing: 1 },
  ...
  tierTagText: { fontFamily: 'Inter_700Bold', fontSize: 10, letterSpacing: 1 },
  ...
  auraTagText: { fontFamily: 'Inter_600SemiBold', fontSize: 11, color: COLORS.gold },
  ...
  liftoffBtnText: { fontFamily: 'Inter_700Bold', fontSize: 13, color: COLORS.background },
