...
import { useGame, apiGet, apiPost, Character } from '@/context/GameContext';
...
function DailyQuestCard({
  ...
  quest: DailyQuest;
  onComplete: (payload: { xp: number; coins: number; title: string; fullScreenConfetti: boolean; soundType: 'levelup' | 'coin'; character?: Character | null }) => void;
}) {
  ...
  async function handleComplete() {
    if (quest.isClaimed || completing) return;
    if (quest.currentProgress < quest.targetValue) return;
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy);
    setCompleting(true);
    try {
      const res = await apiPost<any>(`/quests/${quest.id}/complete`, {});
      if (!res.ok) {
      ...
      const nextTier = String((res.data as any)?.character?.tier || '').toLowerCase();
      const prevTier = String((res.data as any)?.previousTier || '').toLowerCase();
      const tierChanged = !!nextTier && !!prevTier && nextTier !== prevTier;
      const title = quest.title.toLowerCase() === 'iron will'
        ? 'DEMIR IRADE KAZANILDI!'
        : tierChanged
          ? `KADEME ATLADIN: ${nextTier.toUpperCase()}!`
          : 'Tebrikler Usta!';
      onComplete({
        xp: Number((res.data as any)?.xpEarned ?? quest.xpReward ?? 0),
        coins: Number((res.data as any)?.coinEarned ?? quest.coinReward ?? 0),
        title,
        fullScreenConfetti: nextTier === 'mythic' && tierChanged,
        soundType: tierChanged ? 'levelup' : 'coin',
        character: ((res.data as any)?.character || null) as Character | null,
      });
    } catch (e) {
      console.error(e);
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
  ...
  const isReady = quest.currentProgress >= quest.targetValue && !quest.isClaimed;
  const isDone = quest.isClaimed || showSuccess;
          ...
          >
            {completing ? (
              ...
              <>
                ...
                <Text style={styles.completeBtnText}>Ödülü Al</Text>
              </>
            )}
          </Pressable>
        )}
          ...
          <Pressable
            ...
            onPress={() =>
              router.push({
                ...
                params: {
                  exerciseType: quest.exerciseType === 'custom' ? 'push_up' : quest.exerciseType,
                  exerciseName: quest.title,
                },
              })
            }
          >
            <MaterialCommunityIcons name="arrow-right-bold-circle-outline" size={15} color={COLORS.info} />
...
export default function QuestsScreen() {
  ...
  const { userId, refreshCharacter, setCharacter } = useGame();
  ...
  const onRefresh = useCallback(async () => {
    ...
    setRefreshing(false);
  }, [queryClient, refetch, refreshCharacter, userId]);

  const handleComplete = useCallback(async (payload: { xp: number; coins: number; title: string; fullScreenConfetti: boolean; soundType: 'levelup' | 'coin'; character?: Character | null }) => {
    ...
    setRewardToast({
      ...
      coins: payload.coins,
      title: payload.title,
      fullScreenConfetti: payload.fullScreenConfetti,
      soundType: payload.soundType,
    });
  }, [queryClient, userId, refreshCharacter, setCharacter]);
  ...
  return (
    <View style={styles.screen}>
      <RewardToast
        ...
        onHide={() => setRewardToast((p) => ({ ...p, visible: false, fullScreenConfetti: false }))}
      />
      <ScrollView
      ...
      <Text style={styles.pageSubtitle}>Seed edilen ozel gorevleri tamamla, XP ve Coin kazan</Text>
...
const styles = StyleSheet.create({
  ...
  goBtnText: { fontFamily: 'Inter_700Bold', fontSize: 13, color: COLORS.info },
