...
export async function updateDailyQuestProgress(
  ...
  if (quests.length === 0) {
    ...
    if (missing.length > 0) {
      ...
      for (const d of missing) {
        ...
        await db.insert(dailyQuestsTable).values({
          userId, questDate: today, difficulty: d,
  ...
  for (const quest of incomplete) {
    const currentProgress = Number(quest.currentProgress ?? quest.progress ?? 0);
    const targetValue = Number(quest.targetValue ?? quest.target ?? 0);
    const isCompleted = Boolean(quest.isCompleted ?? quest.completed ?? false);
    ...
    const nowCompleted = newProgress >= targetValue;
    if (newProgress === currentProgress && nowCompleted === isCompleted) continue;
    await db.execute(
      sql`UPDATE daily_quests
...
export async function updateBoostQuestProgress(userId: string, timezone: string) {
  ...
  for (const quest of boostQuests) {
    ...
    const targetValue = Number(quest.targetValue ?? quest.target ?? 0);
    const isCompleted = Boolean(quest.isCompleted ?? quest.completed ?? false);
    const newProgress = Math.min(currentProgress + 1, targetValue);
    const nowCompleted = newProgress >= targetValue;
    if (newProgress === currentProgress && nowCompleted === isCompleted) continue;
    await db.execute(
      sql`UPDATE daily_quests
