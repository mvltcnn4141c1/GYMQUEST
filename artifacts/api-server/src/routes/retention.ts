...
router.post("/retention/claim-daily", authenticateUser, rateLimiter, async (req, res) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!isDebugUser && char.last_daily_reward_date === today) {
...
router.post("/retention/claim-weekly-chest", authenticateUser, rateLimiter, async (req, res) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!isDebugUser && weekly.daysActive < 7) throw { status: 400, error: `7 gun aktif olmaniz gerekiyor (${weekly.daysActive}/7)` };
      if (!isDebugUser && weekly.chestClaimed) throw { status: 400, error: "Haftalik sandik zaten alindi", code: "ALREADY_CLAIMED" };
...
router.post("/retention/claim-comeback", authenticateUser, rateLimiter, async (req, res) => {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!isDebugUser && !char.last_workout_at) throw { status: 400, error: "Henuz antrenman yapmamissiniz" };

      const hoursSinceLast = char.last_workout_at
        ? (now.getTime() - new Date(char.last_workout_at).getTime()) / (60 * 60 * 1000)
        : COMEBACK_MIN_HOURS;
      if (!isDebugUser && hoursSinceLast < COMEBACK_MIN_HOURS) {
        throw { status: 400, error: "Geri donus odulu icin en az 48 saat inaktif olmaniz gerekiyor" };
      }

      if (!isDebugUser && char.last_comeback_claim_at) {
...
export async function trackDailyActivity(userId: string, timezone: string) {
  ...
  await db.update(weeklyActivityTable).set({
    workoutsDone: (existing.workoutsDone || 0) + 1,
    updatedAt: new Date(),
