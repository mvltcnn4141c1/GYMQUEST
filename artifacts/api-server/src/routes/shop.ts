...
async function seedProducts() {
  for (const p of PRODUCT_CATALOG) {
    const values = {
      ...
      gemAmount: p.gemsAmount + p.bonusGems,
      coinAmount: 0,
      isActive: 1,
  ...
  try {
    ...
    res.json({
      products: products.map((p) => ({
        ...
        description: p.description || "",
        type: "gem_pack",
        priceUSD: Math.round(Number(p.priceUsd || 0) * 100),
        originalPriceUSD: null,
        priceDisplay: `$${Number(p.priceUsd || 0).toFixed(2)}`,
        originalPriceDisplay: null,
        gemsAmount: p.gemAmount,
        bonusGems: 0,
        totalGems: p.gemAmount,
        includesBattlePass: false,
        includesBoost: null,
        includesAura: null,
        tag: null,
  ...
  try {
      ...
      if (existingPurchase) {
        res.json({
          ...
          product: {
            id: product.id,
              name: product.title,
              priceUSD: Math.round(Number(product.priceUsd || 0) * 100),
              priceDisplay: `$${Number(product.priceUsd || 0).toFixed(2)}`,
          },
          alreadyExists: true,
        });
        return;
      ...
      if (idempotencyKey && insertErr?.code === "23505") {
        ...
        if (existing) {
          res.json({
            ...
            product: { id: product.id, name: product.title, priceUSD: Math.round(Number(product.priceUsd || 0) * 100), priceDisplay: `$${Number(product.priceUsd || 0).toFixed(2)}` },
            alreadyExists: true,
          });
          return;
        }
    ...
    res.json({
      ...
      product: {
        ...
        priceUSD: Math.round(Number(product.priceUsd || 0) * 100),
        priceDisplay: `$${Number(product.priceUsd || 0).toFixed(2)}`,
        gemsAmount: product.gemAmount,
        bonusGems: 0,
        totalGems: product.gemAmount,
        includesBattlePass: false,
      },
      alreadyExists: false,
    });
  ...
  try {
    ...
    res.json({
      offer: {
        ...
        productName: product?.title || offer.productId,
        originalPriceUSD: Math.round(Number(product?.priceUsd || 0) * 100),
        originalPriceDisplay: product ? `$${Number(product.priceUsd || 0).toFixed(2)}` : null,
        discountPercent: offer.discountPercent,
        discountedPriceUSD: offer.discountedPriceUSD,
        discountedPriceDisplay: `$${(offer.discountedPriceUSD / 100).toFixed(2)}`,
        expiresAt: offer.expiresAt,
        gemsAmount: product?.gemAmount || 0,
        bonusGems: 0,
        totalGems: product?.gemAmount || 0,
      },
    });
  } catch (err) {
