...
import { charactersTable, purchasesTable, activeBoostsTable, itemsTable } from "@workspace/db/schema";
...
router.get("/store", authenticateUser, async (req, res) => {
  ...
  const isAdmin = req.user?.isDebugUser === true || req.user?.isAdmin === true;
  ...
  const todayBoostPurchases = purchases.filter(
    (p) => {
      ...
      return item?.type === "boost" && new Date(p.createdAt) >= todayStart;
    }
  );
  ...
  const dbCatalog = dbItems.map((item) => ({
    id: item.id,
    name: item.name,
...
router.post("/store/purchase", authenticateUser, rateLimiter, purchaseRateLimiter, validateUserAction, async (req, res) => {
  ...
  const [dbItem] = await db.select().from(itemsTable).where(and(eq(itemsTable.id, itemId), eq(itemsTable.isActive, true)));
  if (!item && !dbItem) {
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      if (!isAdmin && item && char.level < item.levelRequired) {
        throw new Error(`Bu urun icin seviye ${item.levelRequired} gereklidir`);
      }

      if (!isAdmin && dbItem) {
        if ((char.gym_coins || 0) < dbItem.priceCoins) throw new Error("Yeterli Gym Coin yok");
      } else if (!isAdmin && item.currency === "gym_coin") {
      ...
      if (!isAdmin && item && !item.isConsumable) {
      ...
      if (item?.type === "boost" && !isAdmin) {
        ...
        const todayPurchases = await tx
          ...
          .where(and(
            ...
            gt(purchasesTable.createdAt, todayStart),
          ));
        ...
        if (lastBoost.length > 0) {
          ...
          if (lastItem?.type === "boost") {
            const elapsed = Date.now() - new Date(lastBoost[0].createdAt).getTime();
      ...
      const purchaseCurrency = dbItem ? "gym_coin" : item!.currency;
      const purchasePrice = isAdmin ? 1 : (dbItem ? dbItem.priceCoins : item!.price);
      const purchaseType = dbItem ? "item" : item!.type;
      ...
      const debitField = isAdmin
        ? {}
        : purchaseCurrency === "gym_coin"
        ? { gymCoins: sql`GREATEST(0, ${charactersTable.gymCoins} - ${purchasePrice})` }
        : { gems: sql`GREATEST(0, ${charactersTable.gems} - ${purchasePrice})` };
      ...
      // DB item effect: Protein Shake grants +20% XP boost
      if (dbItem?.id === "itm_protein_shake") {
        await tx.delete(activeBoostsTable).where(eq(activeBoostsTable.userId, userId));
