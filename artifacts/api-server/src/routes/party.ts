...
export async function applyBattleRewards(winnerClanId: string | number) {
  const clanId = Number(winnerClanId);
  const now = new Date();
  const boostEndsAt = new Date(now.getTime() + 24 * 60 * 60 * 1000);
  const visualEndsAt = new Date(now.getTime() + 12 * 60 * 60 * 1000);

  const members = await db
    .select()
    .from(partyMembersTable)
    .where(eq(partyMembersTable.partyId, clanId));

  for (const member of members) {
    await db.insert(activeBoostsTable).values({
      userId: member.userId,
      boostType: "clan_war_xp_20",
      multiplier: 120,
      startsAt: now,
    ...
    await db.update(charactersTable).set({
      championBannerUntil: boostEndsAt,
      eliteTierUntil: visualEndsAt,
      updatedAt: now,
    }).where(eq(charactersTable.userId, member.userId));
  }

  await db.update(partiesTable).set({
    clanWarWins: sql`${partiesTable.clanWarWins} + 1`,
    clanBattlePoints: sql`${partiesTable.clanBattlePoints} + 100`,
    championBannerUntil: boostEndsAt,
    updatedAt: now,
  }).where(eq(partiesTable.id, clanId));
}
