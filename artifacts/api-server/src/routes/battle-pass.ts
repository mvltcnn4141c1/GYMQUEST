...
async function getOrCreateUserPass(userId: string, seasonId: string | number) {
  const seasonIdValue = Number(seasonId);
  const seasonIdText = seasonId.toString();
  const [existing] = await db
    .select()
    .from(userBattlePassTable)
    .where(and(
      eq(userBattlePassTable.userId, userId),
      eq(userBattlePassTable.seasonId, seasonIdValue)
    ));
...
export async function addBattlePassXp(userId: string, xpAmount: number) {
  ...
  try {
    ...
    let currentXp = row.xp as number;
    let currentLevel = row.level as number;
