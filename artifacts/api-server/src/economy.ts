...
async function atomicAddAndCap(
  ...
  const safeCoinsDelta = Math.max(0, Number(coinsDelta) || 0);
  const safeGemsDelta = Math.max(0, Number(gemsDelta) || 0);
  const safeXpDelta = Math.max(0, Number(xpDelta) || 0);
  const safeWorkoutDelta = Math.max(0, Number(workoutDelta) || 0);
  const safeDailyCoinCap = Number(DAILY_COIN_CAP) || 0;
  const safeDailyGemCap = Number(DAILY_GEM_CAP) || 0;
  ...
  const rows = await db.execute(
    sql`INSERT INTO daily_economy (id, user_id, economy_date, coins_earned, gems_earned, xp_earned, workout_count, updated_at)
        VALUES (${id}, ${userId}, ${dateStr},
          LEAST(CAST(${safeCoinsDelta} AS INTEGER), CAST(${safeDailyCoinCap} AS INTEGER)),
          LEAST(CAST(${safeGemsDelta} AS INTEGER), CAST(${safeDailyGemCap} AS INTEGER)),
          CAST(${safeXpDelta} AS INTEGER), CAST(${safeWorkoutDelta} AS INTEGER), NOW())
        ON CONFLICT (user_id, economy_date) DO UPDATE SET
          coins_earned = LEAST(daily_economy.coins_earned + CAST(${safeCoinsDelta} AS INTEGER), CAST(${safeDailyCoinCap} AS INTEGER)),
          gems_earned = LEAST(daily_economy.gems_earned + CAST(${safeGemsDelta} AS INTEGER), CAST(${safeDailyGemCap} AS INTEGER)),
          xp_earned = daily_economy.xp_earned + CAST(${safeXpDelta} AS INTEGER),
          workout_count = daily_economy.workout_count + CAST(${safeWorkoutDelta} AS INTEGER),
          updated_at = NOW()
        RETURNING coins_earned, gems_earned`
  );

  const row = rows.rows?.[0] as any;
  const newCoinsEarned = Number(row?.coins_earned ?? safeCoinsDelta);
  const newGemsEarned = Number(row?.gems_earned ?? safeGemsDelta);

  const prevCoins = newCoinsEarned - Math.min(safeCoinsDelta, safeDailyCoinCap - Math.max(0, newCoinsEarned - safeCoinsDelta));
  const actualCoinsAdded = newCoinsEarned - Math.max(0, prevCoins);
  const prevGems = newGemsEarned - Math.min(safeGemsDelta, safeDailyGemCap - Math.max(0, newGemsEarned - safeGemsDelta));
  const actualGemsAdded = newGemsEarned - Math.max(0, prevGems);

  return {
    newCoinsEarned,
    newGemsEarned,
    actualCoinsAdded: Math.min(safeCoinsDelta, Math.max(0, actualCoinsAdded)),
    actualGemsAdded: Math.min(safeGemsDelta, Math.max(0, actualGemsAdded)),
  };
}
